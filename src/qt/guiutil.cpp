...
#define URI_SCHEME "advantagecredit"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a AdvantageCoin address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
bool parseBitcoinURI(const QUrl& uri, SendCreditsRecipient* out)
{
    // return if URI is not valid or is no AdvantageCoin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::USDA, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBitcoinURI(QString uri, SendCreditsRecipient* out)
{
    // Convert advantagecredit:// to advantagecredit:
    //
    //    Cannot handle this later, because advantagecredit:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCreditsRecipient& info)
{
    QString ret = QString(URI_SCHEME ":%1").arg(info.address);
    int paramCount = 0;

    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::USDA, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open advantagecredit.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "AdvantageCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for AdvantageCoin.lnk
...
{
    return GetAutostartDir() / "advantagecredit.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a advantagecredit.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=AdvantageCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the advantagecredit app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add advantagecredit app to startup item list
