...
CAmount WalletModel::getBalance(const CCreditControl* coinControl) const
{
    if (coinControl) {
        CAmount nBalance = 0;
        std::vector<COutput> vCredits;
        wallet->AvailableCredits(vCredits, true, coinControl);
...
WalletModel::SendCreditsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCreditControl* coinControl)
{
    CAmount total = 0;
    ...
    foreach (const SendCreditsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered advantagecredit address / amount:
...
WalletModel::SendCreditsReturn WalletModel::sendCredits(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCreditsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal advantagecredit:URI (advantagecredit:XyZ...?message=example)
            {
