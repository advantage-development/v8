...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::USDA)
    {
    }
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeAdvantageCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAdvantageCoinAmount * CREDIT, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeAdvantageCoinAmount = strAnonymizeAdvantageCoinAmount.remove(strAnonymizeAdvantageCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAdvantageCoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeAdvantageCoinAmount * CREDIT) nMaxToAnonymize = nAnonymizeAdvantageCoinAmount * CREDIT;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeAdvantageCoinAmount * CREDIT) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeAdvantageCoinAmount));
        strAnonymizeAdvantageCoinAmount = strAnonymizeAdvantageCoinAmount.remove(strAnonymizeAdvantageCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAdvantageCoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeAdvantageCoinAmount == 0) {
