...
#include "config/advantagecredit-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("advantagecredit-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "advantagecredit.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "advantagecreditd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"),  9797, 39795));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcredits, mempool, net, advantagecredit, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCREDIT_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all AdvantageCoin specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    if (GetBoolArg("-help-debug", false)) {
        ...
        strUsage += HelpMessageOpt("-printcreditstake", _("Display verbose coin stake messages in the debug.log file."));
    }
#endif
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235: 9797"));
    ...
    strUsage += HelpMessageOpt("-anonymizeadvantagecreditamount=<n>", strprintf(_("Keep N USDA anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"),  9898, 39799));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(_("Copyright (C) 2018 The AdvantageCoin Core Developers")) + "\n" +
           "\n" +
...
{
    RenameThread("advantagecredit-loadblk");
...
/** Sanity checks
 *  Ensure that AdvantageCoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. AdvantageCoin Core is shutting down."));
...
#endif
    // Make sure only a single AdvantageCoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. AdvantageCoin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("AdvantageCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of AdvantageCoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart AdvantageCoin Core to complete") << "\n";
    ...
    nAnonymizeAdvantageCoinAmount = GetArg("-anonymizeadvantagecreditamount", 0);
    if (nAnonymizeAdvantageCoinAmount > 999999) nAnonymizeAdvantageCoinAmount = 999999;
    if (nAnonymizeAdvantageCoinAmount < 2) nAnonymizeAdvantageCoinAmount = 2;
    ...
    LogPrintf("Anonymize AdvantageCoin Amount %d\n", nAnonymizeAdvantageCoinAmount);
