...
#if defined(NDEBUG)
#error "AdvantageCoin cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 1 * 30 * 60;
...
CCreditsViewCache* pcreditsTip = NULL;
...
unsigned int GetP2SHSigOpCount(const CTransaction& tx, const CCreditsViewCache& inputs)
{
    if (tx.IsCoinBase())
        return 0;
...
int GetInputAge(CTxIn& vin)
{
    ...
    {
        LOCK(mempool.cs);
        CCreditsViewMemPool viewMempool(pcreditsTip, mempool);
        view.SetBackend(viewMempool); // temporarily switch cache backend to db+mempool view

        const CCredits* credits = view.AccessCredits(vin.prevout.hash);
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    //Creditstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
    ...
    {
        CCreditsView dummy;
        CCreditsViewCache view(&dummy);

        CAmount nValueIn = 0;
        {
            LOCK(pool.cs);
            CCreditsViewMemPool viewMemPool(pcreditsTip, pool);
            view.SetBackend(viewMemPool);

            // do we already have it?
            if (view.HaveCredits(hash))
                return false;
    ...
    CBlockIndex* pindexSlow = NULL;
    {
        ...
        if (fAllowSlow) { // use coin database to locate block that contains transaction, and scan it
            int nHeight = -1;
            {
                ...
                const CCredits* credits = view.AccessCredits(hash);
                if (credits)
...
void UpdateCredits(const CTransaction& tx, CValidationState& state, CCreditsViewCache& inputs, CTxUndo& txundo, int nHeight)
{
    ...
    if (!tx.IsCoinBase()) {
        ...
        BOOST_FOREACH (const CTxIn& txin, tx.vin) {
            ...
            bool ret = inputs.ModifyCredits(txin.prevout.hash)->Spend(txin.prevout, txundo.vprevout.back());
            assert(ret);
        }
    }

    // add outputs
    inputs.ModifyCredits(tx.GetHash())->FromTx(tx, nHeight);
}
...
bool CheckInputs(const CTransaction& tx, CValidationState& state, const CCreditsViewCache& inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck>* pvChecks)
{
    if (!tx.IsCoinBase()) {
        if (pvChecks)
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++) {
            ...
            const CCredits* credits = inputs.AccessCredits(prevout.hash);
            assert(credits);
            ...
            if (credits->IsCoinBase() || credits->IsCoinStake()) {
                if (nSpendHeight - credits->nHeight < Params().CREDITBASE_MATURITY())
                    return state.Invalid(
                        error("CheckInputs() : tried to spend coinbase at depth %d, creditstake=%d", nSpendHeight - credits->nHeight, credits->IsCoinStake()),
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCreditsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        {
            CCredits outsEmpty;
            CCreditsModifier outs = view.ModifyCredits(hash);
            outs->ClearUnspendable();

            CCredits outsBlock(tx, pindex->nHeight);
            // The CCredits serialization does not serialize negative numbers.
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                CCreditsModifier credits = view.ModifyCredits(out.hash);
                if (undo.nHeight != 0) {
...
{
    RenameThread("advantagecredit-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCreditsViewCache& view, bool fJustCheck)
{
    AssertLockHeld(cs_main);
    ...
    if (fEnforceBIP30) {
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
            const CCredits* credits = view.AccessCredits(tx.GetHash());
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        UpdateCredits(tx, state, view, i == 0 ? undoDummy : blockundo.vtxundo.back(), pindex->nHeight);
...
bool static FlushStateToDisk(CValidationState& state, FlushStateMode mode)
{
    ...
    try {
        if ((mode == FLUSH_STATE_ALWAYS) ||
            ...
            // Typical CCredits structures on disk are around 100 bytes in size.
...
bool DisconnectBlockAndInputs(CValidationState& state, CTransaction txLock)
{
    ...
    //    CCreditsViewCache view(*pcreditsTip, true);
...
bool TestBlockValidity(CValidationState& state, const CBlock& block, CBlockIndex* const pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    CCreditsViewCache viewNew(pcreditsTip);
...
bool static LoadBlockIndexDB()
{
    ...
    if (!fLastShutdownWasPrepared && !GetBoolArg("-forcestart", false) && !GetBoolArg("-reindex", false) && (vSortedByHeight.size() != vinfoBlockFile[nLastBlockFile].nHeightLast + 1) && (vinfoBlockFile[nLastBlockFile].nHeightLast != 0)) {
        ...
        CBlockIndex* pindexCreditsView = mapBlockIndex[pcreditsTip->GetBestBlock()];
        for(unsigned int i = vinfoBlockFile[nLastBlockFile].nHeightLast + 1; i < vSortedByHeight.size(); i++)
        {
            ...
            if(pindexLastMeta->nHeight > pindexCreditsView->nHeight)
                break;
        }
...
bool CVerifyDB::VerifyDB(CCreditsView* creditsview, int nCheckLevel, int nCheckDepth)
{
    LOCK(cs_main);
    ...
    CCreditsViewCache credits(creditsview);
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_TX: {
        ...
        return txInMap || mapOrphanTransactions.count(inv.hash) ||
               pcreditsTip->HaveCredits(inv.hash);
    }
    case MSG_DSTX:
